...
class EDVRDarts(nn.Module):
    ...
    def __init__(self,
                 C, N, max_nodes, search_space, affine, track_running_stats,
                 num_in_ch=3,
                 num_out_ch=3,
                 num_feat=64,
                 num_frame=5,
        ...
        #--------------Cells------------------
        # self.feature_extraction = make_layer(
        #     SearchCell, num_extract_block, 
        #     num_feat, num_feat, 1, max_nodes, search_space, affine, track_running_stats
        # )
        #-------------------------------------
        num_edge, edge2index = None, None

        self.feature_extraction_cells = nn.ModuleList()

        for i in range(num_extract_block):
            cell = SearchCell(num_feat, num_feat, 1, max_nodes, search_space, affine, track_running_stats)
            if num_edge is None: num_edge, edge2index = cell.num_edges, cell.edge2index
            else: assert num_edge == cell.num_edges and edge2index == cell.edge2index, 'invalid {:} vs. {:}.'.format(num_edge, cell.num_edges)

            self.feature_extraction_cells.append(cell)
        #-------------------------------------
        ...
        #--------------Cells------------------
        # self.reconstruction = make_layer(
        #     SearchCell, num_reconstruct_block,
        #     num_feat, num_feat, 1, max_nodes, search_space, affine, track_running_stats
        # )
        #-------------------------------------

        self.reconstruction_cells = nn.ModuleList()

        for i in range(num_reconstruct_block):
            cell = SearchCell(num_feat, num_feat, 1, max_nodes, search_space, affine, track_running_stats)
            if num_edge is None: num_edge, edge2index = cell.num_edges, cell.edge2index
            else: assert num_edge == cell.num_edges and edge2index == cell.edge2index, 'invalid {:} vs. {:}.'.format(num_edge, cell.num_edges)

            self.reconstruction_cells.append(cell)
        #-------------------------------------
        ...
        self._Layer     = len(self.feature_extraction_cells) + len(self.reconstruction_cells)
    ...
    def forward(self, x):
        ...
        #-----feature_extraction_forward------
        for i, cell in enumerate(self.feature_extraction_cells):
            feat_l1 = cell(feat_l1, alphas)
        #-------------------------------------
        ...
        for i, cell in enumerate(self.reconstruction_cells):
            feat = cell(feat, alphas)
        out = feat
        #-------------------------------------
    ...
    def get_weights(self):
        if self.with_predeblur:
            xlist = list( self.predeblur.parameters() )
            xlist += list ( self.conv_1x1.parameters() )
        else:
            xlist = list( self.conv_first.parameters() )

        xlist += list( self.feature_extraction_cells.parameters() )

        xlist += list( self.conv_l2_1.parameters() )
        xlist += list( self.conv_l2_2.parameters() )
        xlist += list( self.conv_l3_1.parameters() )
        xlist += list( self.conv_l3_1.parameters() )
        xlist += list( self.pcd_align.parameters() )
        xlist += list( self.fusion.parameters() )
        ...
        xlist += list( self.lrelu.parameters() )
    ...
    def get_message(self):
        ...
        for i, cell in enumerate(self.feature_extraction_cells):
            string += '\n {:02d}/{:02d} :: {:}'.format(i, len(self.feature_extraction_cells), cell.extra_repr())
        for i, cell in enumerate(self.reconstruction_cells):
            string += '\n {:02d}/{:02d} :: {:}'.format(i, len(self.reconstruction_cells), cell.extra_repr())
        return string
